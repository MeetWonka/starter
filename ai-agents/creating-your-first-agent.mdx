...
<Warning>
Agents can only access tools explicitly assigned to them and inherit your personal permissions.
</Warning>
...
<Tabs>
<Tab title="Basic Agent Creation">

### Basic Agent:

Watch how to create a basic agent with straightforward instructions—perfect for learning the fundamentals of agent creation.
...
<Tab title="Advanced Agent Creation">

### Multi-Tool Agent: 

Watch how to create an agent that connects multiple tools to automate workflows—a practical example of cross-system integration.
...
After deployment, follow this refinement cycle:

<Steps>
<Step title="Monitor agent performance">
Track conversation logs, completion rates, and user feedback to identify where the agent struggles or succeeds.
</Step>

<Step title="Identify recurring problems">
Look for patterns in failures, such as misunderstood intents, incorrect tool usage, or inconsistent responses.
</Step>

<Step title="Update instructions">
Refine your system prompt to address specific issues—add clarifying examples, adjust decision-making rules, or refine tool usage guidelines.
</Step>

<Step title="Test changes thoroughly">
Run updated agents through the same scenarios that previously caused issues to verify improvements.
</Step>

<Step title="Document learnings">
Keep notes on what worked and what didn't to inform future agent development and team knowledge.
</Step>
</Steps>
...
Fine-tune agent behavior with these parameters. Defaults work for most cases—adjust only when needed.
...
**Recommended values:**
- **0.0-0.3:** Factual tasks, data extraction, precise answers—produces consistent, predictable, and focused responses
- **0.7-1.2:** Creative writing, brainstorming, varied responses—generates more creative, diverse, and unpredictable outputs
...
**What it does:** Alternative to temperature—limits word choices to most probable options
...
Start with minimal permissions and test thoroughly before expanding capabilities. Never deploy an agent with full permissions on the first try—unexpected behavior is common and should be caught in testing.
