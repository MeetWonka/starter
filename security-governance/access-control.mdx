...
<Info>
Access control implements the principle of least privilege—users and agents receive only the permissions they need to do their work.
</Info>
...
When creating an AI agent, you can control exactly which tools it has access to—ensuring agents only have the capabilities they need to perform their intended function.
...
**Example**: A data analysis agent doesn't need tools to modify or delete data—only read access.
</Accordion>
...
**Example**: A "Draft Email" agent with Outlook MCP but only drafting tools enabled—no sending, deleting, or forwarding capabilities.
</Accordion>
</AccordionGroup>
